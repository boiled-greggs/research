...
if __name__ == "__main__":
    ...
    out.write("path = [ [0., 0., 0.], [1./3., 1./3., 0.], [0.5, 0., 0.], [0., 0., 0.] ]\n")
    out.write("label = (r'$\Gamma $',r'$K$', r'$M$', r'$\Gamma$')\n")
    out.write("nk = 1000\n\n")
    ...
    out.write("out = open('" + structure + "_bands2.txt', 'w')\n")
    out.write("nodes = open('" + structure + "_nodes2.txt', 'w')\n")
    # get eigenvalues
    out.write("evals = my_model.solve_all(k_vec)\n\n")

    # get band structure figure

    out.write("fig, ax = plt.subplots(figsize=(4,5))\n\n")
    ...
    out.write("\tfor k in range(len(k_dist)):\n")
    out.write("\t\tout.write(str(k_dist[k]) + '\\t' + str(evals[xband][k]) + '\\n')\n")
    out.write("\tax.plot(k_dist, evals[xband], linewidth=.5, color='black')\n")
    out.write("\tout.write('\\n')\n")
    out.write("\n# fig.tight_layout()\n")
    out.write("for n in range(len(k_node)):\n")
    out.write("\tnodes.write(label[n] + '\\t' + str(k_node[n]) + '\\n')\n")
    out.write("fig.savefig('"+structure+"-2.eps')\n\n")
    
    out.write("k_vec = my_model.k_uniform_mesh([7, 7, 7])\n")
    out.write("evals2 = my_model.solve_all(k_vec)\n")
    out.write("\n")
    out.write("evals2 = evals2.flatten()\n")
    out.write("\n")
    out.write("print('Plotting DoS...')\n")
    out.write("\n")
    out.write("fig, ax = plt.subplots()\n")
    out.write("ax.hist(evals2, 80,range=(-4.,4.))\n")
    out.write("#ax.set_ylim(0.0,80.0)\n")
    out.write("# put title\n")
    out.write("ax.set_title('"+structure+" density of states')\n")
    out.write("ax.set_xlabel('Band energy')\n")
    out.write("ax.set_ylabel('Number of states')\n")
    out.write("# make an PDF figure of a plot\n")
    out.write("fig.tight_layout()\n")
    out.write("fig.savefig('"+structure+"_dos2.pdf')\n")
